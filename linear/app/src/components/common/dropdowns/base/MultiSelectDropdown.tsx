import { Check } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
  DropdownMenuTrigger,
  DropdownMenuSeparator,
} from "@/components/ui/dropdown-menu";
import { cn } from "@/lib/utils";
import type { ReactNode } from "react";
import { useState, useEffect } from "react";
import type { DropdownItem, DropdownOption } from "@/lib/types";

interface MultiSelectDropdownProps {
  items: DropdownItem[];
  values: string[];
  onChange: (values: string[]) => void;
  trigger: ReactNode;
  triggerClassName?: string;
  showSearch?: boolean;
  searchPlaceholder?: string;
  headerShortcut?: string;
  align?: "start" | "center" | "end";
  side?: "top" | "bottom" | "left" | "right";
  width?: string;
}

export function MultiSelectDropdown({
  items,
  values,
  onChange,
  trigger,
  triggerClassName,
  showSearch = true,
  searchPlaceholder = "Search...",
  headerShortcut,
  align = "start",
  side = "bottom",
  width = "min-w-[240px]",
}: MultiSelectDropdownProps) {
  const [search, setSearch] = useState("");
  const [open, setOpen] = useState(false);
  const [pendingValues, setPendingValues] = useState(values);

  // Filter options based on search
  const filteredItems = items.filter((item) => {
    if (item.type !== "option") return true;
    if (!search) return true;

    const searchText = item.searchableText || item.label;
    return searchText.toLowerCase().includes(search.toLowerCase());
  });

  // Check if any options were found after filtering
  const hasResults = filteredItems.some((item) => item.type === "option");

  // If no results found, show default view
  const baseItems = !search || hasResults ? filteredItems : items;

  // Separate selected and unselected items
  const selectedItems: DropdownItem[] = [];
  const unselectedItems: DropdownItem[] = [];

  // Use pendingValues for display while dropdown is open
  const displayValues = open ? pendingValues : values;

  baseItems.forEach((item) => {
    if (item.type === "option") {
      if (displayValues.includes(item.value)) {
        selectedItems.push(item);
      } else {
        unselectedItems.push(item);
      }
    } else {
      // Keep separators and labels in their original position
      unselectedItems.push(item);
    }
  });

  // Create display items with selected items first, then separator, then unselected
  const displayItems: DropdownItem[] = [
    ...selectedItems,
    ...(selectedItems.length > 0 && unselectedItems.length > 0
      ? [{ type: "separator" as const }]
      : []),
    ...unselectedItems,
  ];

  const handleToggleValue = (value: string) => {
    if (pendingValues.includes(value)) {
      setPendingValues(pendingValues.filter((v) => v !== value));
    } else {
      setPendingValues([...pendingValues, value]);
    }
  };

  const handleOpenChange = (isOpen: boolean) => {
    if (isOpen) {
      // When opening, sync pendingValues with current values
      setPendingValues(values);
      setOpen(true);
    } else {
      // When closing, first close the dropdown, then commit changes after animation
      setOpen(false);
      // Delay the onChange to let the dropdown close animation complete
      setTimeout(() => {
        onChange(pendingValues);
      }, 150); // Match typical dropdown close animation duration
    }
  };

  // Sync pendingValues when values prop changes externally (e.g., from parent)
  useEffect(() => {
    if (!open) {
      setPendingValues(values);
    }
  }, [values, open]);

  return (
    <DropdownMenu open={open} onOpenChange={handleOpenChange}>
      <DropdownMenuTrigger asChild className={triggerClassName}>
        {trigger}
      </DropdownMenuTrigger>
      <DropdownMenuContent
        align={align}
        side={side}
        className={cn(
          width,
          "bg-card-bg border-border p-0 flex flex-col overflow-hidden"
        )}
      >
        {showSearch && (
          <div className="flex items-center pl-[14px] pr-3 border-b border-border justify-between gap-[14px] flex-shrink-0">
            <input
              type="text"
              placeholder={searchPlaceholder}
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="w-full bg-transparent text-[13px] text-neutral-2 placeholder:text-neutral-5 focus:outline-none pr-6 h-9"
              autoComplete="off"
              onClick={(e) => e.stopPropagation()}
              onKeyDown={(e) => e.stopPropagation()}
            />
            {headerShortcut && (
              <span className="text-[11px] text-neutral-5 h-[18px] w-[18px] flex items-center justify-center border border-border rounded">
                {headerShortcut}
              </span>
            )}
          </div>
        )}

        <div
          className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden [&::-webkit-scrollbar]:hidden [&::-webkit-scrollbar]:w-0"
          style={{ scrollbarWidth: "none" }}
        >
          <div className="h-1 w-full" />

          {displayItems.map((item, index) => {
            if (item.type === "separator") {
              return (
                <DropdownMenuSeparator
                  key={`separator-${index}`}
                  className="my-1"
                />
              );
            }

            if (item.type === "label") {
              return (
                <div
                  key={`label-${index}`}
                  className="px-2 py-1 text-[11px] text-neutral-5 font-medium"
                >
                  {item.text}
                </div>
              );
            }

            // Option item
            const option = item as DropdownOption;
            const isSelected = displayValues.includes(option.value);

            return (
              <div key={option.value} className="px-1">
                <DropdownMenuItem
                  onClick={(e) => {
                    e.stopPropagation();
                    handleToggleValue(option.value);
                    setSearch("");
                  }}
                  className={cn(
                    "flex items-center gap-2 px-2 h-8 text-[13px] text-neutral-2 cursor-pointer rounded hover:bg-hover-4 focus:bg-hover-4 transition-colors group focus:text-neutral-2 focus:[&_svg]:text-neutral-2",
                    isSelected && "bg-hover-3"
                  )}
                >
                  <div
                    className={cn(
                      "w-3.5 h-3.5 rounded border-[0.5px] flex items-center justify-center transition-all duration-200",
                      isSelected
                        ? "bg-checkbox-accent border-checkbox-accent"
                        : "border-transparent group-hover:border-neutral-5/30 group-hover:ring-1 group-hover:ring-checkbox-accent/20"
                    )}
                  >
                    {isSelected && (
                      <Check className="size-3 shrink-0 text-white" />
                    )}
                  </div>
                  {option.icon && (
                    <div className="w-3.5 h-3.5 flex items-center justify-center shrink-0">
                      {option.icon}
                    </div>
                  )}
                  <span className="flex-1">{option.label}</span>
                  {option.shortcut && (
                    <span className="text-[11px] text-neutral-5 w-3">
                      {option.shortcut}
                    </span>
                  )}
                </DropdownMenuItem>
              </div>
            );
          })}

          <div className="h-1 w-full" />
        </div>
      </DropdownMenuContent>
    </DropdownMenu>
  );
}
